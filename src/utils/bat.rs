...
pub async fn render_stdin_with_highlight_range(
    ...
    let columns = sanitize_columns(preview_columns_from_env().unwrap_or(80));
    let lines = sanitize_lines(preview_lines_from_env().unwrap_or(max_lines));
...
fn preview_lines_from_env() -> Option<usize> {
...
fn preview_columns_from_env() -> Option<usize> {
